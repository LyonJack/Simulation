...
struct st_core_frame_corrinfo
{
	...
	e_nav_correct_type type;	// 校准状态,定义见@e_nav_correct_type
...
struct st_hmi_frame_syspara
{
	...
	double latitude;			// 初始纬度
	double longitude;			// 初始经度
	double altitude;			// 初始高度
	e_trjactory_source trj_src;	// 轨迹生成源
	uint32_t sim_speed;			// 仿真倍率
	uint32_t sim_step;			// 仿真步长
	uint32_t task_id;			// 仿真任务ID
};
...
struct st_gyro_para
{
	...
	double mis_zx;				// 陀螺失准角误差(z-x)(arcmin)
    double mis_zy;				// 陀螺失准角误差(z-y)(arcmin)
...
struct st_acce_para
{
	...
	double mis_zx;				// 加计失准角误差(z-x)(arcmin)
    double mis_zy;				// 加计失准角误差(z-y)(arcmin)
...
{
	e_alignment_type type;		// 对准类型,定义详见e_navigation_type
...
{
	e_correction_type type;		// 对准类型,定义详见e_navigation_type
...
{
	st_device device;			// 设备类型,定义见 @st_device
	st_alignment_flowpara alig; // 对准流程参数,定义见 @st_alignment_flowpara
